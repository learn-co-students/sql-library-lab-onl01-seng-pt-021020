...
def select_series_title_with_most_human_characters
  "SELECT title FROM series ORDER BY title LIMIT 1 ;"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(*) AS book_num FROM character_books JOIN characters ON character_books.character_id = characters.id GROUP BY characters.name ORDER BY book_num DESC, characters.name ASC ;"
end
